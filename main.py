...
if st.session_state.page == "upload":
        ...
        with st.container():
            ...
            with col2:
                ...
                if st.button('Make Prediction'):
                    # Prepare data for N-BEATS forecasting
                    model = NBEATS(input_size=30, output_size=7)
                    nf = NeuralForecast(models=[model], freq='D')
                    nf.fit(datasales)

                    future_steps = 7  # Predict next 7 days
                    predicted_values = nf.predict(future_steps)

                    future_dates = pd.date_range(start=datasales.index[-1] + pd.Timedelta(days=1), periods=future_steps, freq='D')
    ...
    with st.container(): 
        ...
        with col32: 
            fig = px.scatter(st.session_state.df, x="Jumlah Produk", y="Harga Produk", color="Kategori", 
                             title="Scatter Plot Berdasarkan Kategori", size_max=10, symbol="Kategori")
            st.plotly_chart(fig)
